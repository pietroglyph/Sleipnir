...
void SimplicialCholeskyBase<Derived>::factorize_preordered(const CholMatrixType& ap) {
  ...
  // find correct signs of diagonal
  Vector<StorageIndex, Dynamic> signs{ap.rows()};
  signs.segment(0, m_positiveEigenvalues).setConstant(1);
  signs.segment(m_positiveEigenvalues, ap.rows() - m_positiveEigenvalues).setConstant(-1);
  if (m_P.size() > 0) {
    // PAP⁻¹ = LDLᵀ, so pivot signs via P to match permuted diagonal
    signs = m_P * signs;
  }
  m_maxRegularization = 0.0;

  Scalar apInfNorm = ap.diagonal().template lpNorm<Infinity>();
  Scalar zeroTol = std::numeric_limits<Scalar>::epsilon() * std::max(1.0, apInfNorm);
  ...
  for (StorageIndex k = 0; k < size; ++k) {
    ...
    if (DoLDLT) {
      if (signs[k] == 1) {
        // regularize value that's supposed to be positive
        if (d < RealScalar(0)) {
          // if it's a negative value, invert it
          m_diag[k] = std::max(zeroTol, -d);
        } else if (d == RealScalar(0)) {
          // if it's zero, bump it to 1
          m_diag[k] = apInfNorm > 0.0 ? 1.0 : std::numeric_limits<Scalar>::epsilon();
        } else {
          // if it's a positive value, copy it as is
          m_diag[k] = d;
        }
      } else {
        // regularize value that's supposed to be negative
        if (d > RealScalar(0)) {
          // if it's a positive value, invert it
          m_diag[k] = std::min(-zeroTol, -d);
        } else if (d == RealScalar(0)) {
          // if it's zero, bump it to -1
          m_diag[k] = apInfNorm > 0.0 ? -1.0 : -std::numeric_limits<Scalar>::epsilon();
        } else {
          // if it's a negative value, copy it as is
          m_diag[k] = d;
        }
      }
      m_maxRegularization = std::max(m_maxRegularization, std::abs(m_diag[k] - d));
    } else {
