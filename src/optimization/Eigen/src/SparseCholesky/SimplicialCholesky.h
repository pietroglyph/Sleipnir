...
class SimplicialCholeskyBase : public SparseSolverBase<Derived> {
 ...
 public:
  ...
  explicit SimplicialCholeskyBase(const MatrixType& matrix, Index positiveEigenvalues)
      : m_info(Success), m_factorizationIsOk(false), m_analysisIsOk(false), m_positiveEigenvalues(positiveEigenvalues), m_shiftOffset(0), m_shiftScale(1) {
  ...
  DiagonalScalar getMaxRegularization() const { return m_maxRegularization; }
 ...
 protected:
  ...
  void compute(const MatrixType& matrix, Index positiveEigenvalues) {
    eigen_assert(matrix.rows() == matrix.cols());
    m_positiveEigenvalues = positiveEigenvalues;
  ...
  template <bool DoLDLT, bool NonHermitian>
  void factorize(const MatrixType& a, Index positiveEigenvalues) {
  ...
  Index m_positiveEigenvalues;
  DiagonalScalar m_shiftOffset;
  DiagonalScalar m_shiftScale;
  DiagonalScalar m_maxRegularization = 0.0;
};
...
class SimplicialLDLT : public SimplicialCholeskyBase<SimplicialLDLT<MatrixType_, UpLo_, Ordering_> > {
 ...
 public:
  ...
  /** Constructs and performs the LLT factorization of \a matrix */
  explicit SimplicialLDLT(const MatrixType& matrix, Index positiveEigenvalues) : Base(matrix, positiveEigenvalues) {}
  ...
  /** Computes the sparse Cholesky decomposition of \a matrix */
  SimplicialLDLT& compute(const MatrixType& matrix, int positiveEigenvalues) {
    Base::template compute<true, false>(matrix, positiveEigenvalues);
    return *this;
  }
   ...
   * \sa analyzePattern()
   */
  void factorize(const MatrixType& a, Index positiveEigenvalues) { Base::template factorize<true, false>(a, positiveEigenvalues); }
